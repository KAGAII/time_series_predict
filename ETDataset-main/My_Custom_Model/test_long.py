...
def train(net, train_iter, valid_iter, epochs, learning_rate, device, scaler_model):
    ...
    for epoch in range(epochs):
        ...
        for x_train, _, y_train, x_mask, _, y_mask in train_bar:
            ...
            y_train_pred = net(x_train, x_mask).to(device)
        ...
        with torch.no_grad():
            ...
            for x_valid, _, y_valid, x_mask, _, y_mask in valid_iter:
                ...
                y_train_pred = net(x_valid, x_mask).to(device)
            ...
            if bast_loss > valid_loss:
                # 训练完毕之后保存模型
                torch.save(net, f'../output/{save_path}/diy_model_mse.pth')  # 保存模型
...
def predict(net, test_iter,device):
    ...
    with torch.no_grad():
        ...
        for x_test, _, y_test,x_mask, _, y_mask in test_iter:
            ...
            x_mask = x_mask.to(device)
            y_test_pred = net(x_test,x_mask)  # 没有强制教学
...
if __name__ == '__main__':
    ...
    if type_train:
        ...
        model = torch.load(f'../output/{save_path}/diy_model_mse.pth')
        # 模型预测
